...
jQuery(document).ready(function( $ ) {
	...
	$('.popup-youtube, .popup-vimeo, .popup-gmaps').magnificPopup({
		/*disableOn: 700,*/ // We need the mfp-close to unlock our viewport
		type: 'iframe',
		mainClass: 'mfp-fade',
		removalDelay: 160,
		preloader: false,
	...
	$('.open-gallery-link').click(function() {
		...
		var items = [];
		$( $(this).attr('href') ).find('.port-slide').each(function() {
			items.push( {
				src: $(this) 
			} );
		});
        
        $(document).on('click', '.mfp-arrow-left', function(){$('.carousel').carousel(0);});
        $(document).on('click', '.mfp-arrow-right', function(){$('.carousel').carousel(0);});
    ...
    $(document).on('click', '.mfp-close', function(){unlockViewport(); $(window).scrollTop(scrollPosition);});
    
    function lockViewport(){$('html, body').addClass('mfp-lock');}
    
    function unlockViewport(){$('html, body').removeClass('mfp-lock');}
	...
	$("a[href*=#]").bind("click", function(e) {
        ...
        if(!$(this).hasClass('open-gallery-link')){
